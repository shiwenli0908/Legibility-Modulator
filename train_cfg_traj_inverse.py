...
global_cond_dim = (lowdim_obs_dim + target_dim) * obs_horizon + 8 * 2
...
torch.save(ema_nets.state_dict(), "env05_traj_inverse_epoch800.ckpt")
print("EMA model saved to blockreach_ema.ckpt")

# 保存目录
save_dir = "training_plots"
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, "loss_curve_env05_traj_inverse_epoch800.png")

plt.figure(figsize=(8, 5))
plt.plot(all_epoch_losses, label='Train Loss')
plt.xlabel('Epoch')
plt.ylabel('MSE Loss (log scale)')
plt.title('Training Loss Curve for Legibility Diffuser')
plt.yscale('log')
