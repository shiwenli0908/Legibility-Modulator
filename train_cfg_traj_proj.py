...
from diffusion_policy.encoder import PotentialResNet3DEncoder
from diffusion_policy.model import ConditionalUnet1D
from dataset.dataset import BlockReachPotentialSampleDatasetProjection

dataset = BlockReachPotentialSampleDatasetProjection(
    data_dir="test11_3D"
)
...
print(batch['potential'].shape)  # (B, 3, 96, 96)
print(batch['sample'].shape)     # (B, 10, 2)


# construct ResNet18 encoder
potential_encoder = get_resnet('resnet18')
potential_encoder = replace_bn_with_gn(potential_encoder)
...
for epoch_idx in range(num_epochs):
    ...
    for batch in dataloader:
        # ==== 数据准备 ====
        sample_full = batch['sample'].to(device)       # (B, 10, 3)
        start = sample_full[:, 0]                      # (B, 3)
        #target = batch['target'].to(device)            # (B, 3)
        target = sample_full[:, -1]                     # (B, 3)
        score = batch['score'].to(device)              # (B, 1)
        potential = batch['potential'].to(device)      # (B, 1, 96, 96, 96)
        
        # 中间的 8 个点作为生成目标
        sample = sample_full[:, 1:-1]                  # (B, 8, 3)
        B = sample.shape[0]
        ...
        #mask = (torch.rand(B, device=device) < 1).float().view(B, 1)  # (B, 1)
        mask = 0
...
torch.save(ema_nets.state_dict(), "env01_trajectory_potential_proj2_epoch3000.ckpt")
print("EMA model saved to blockreach_ema.ckpt")

# 保存目录
save_dir = "training_plots"
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, "loss_curve_trajectory_potential_proj2_epoch3000.png")
